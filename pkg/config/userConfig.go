...
func initializeDatabaseComponents(log *logrus.Logger) (*gorm.DB, ports.UserPort, *postgres.RoleRepository) {
	...
	return db, userRepository, roleRepository
}
...
func BuildUserCrudDomainHandler(log *logrus.Logger) userRest.CrudRest {
	_, userRepo, roleRepo := initializeDatabaseComponents(log)
	crudService := userService.NewCrudService(log, userRepo, roleRepo)
